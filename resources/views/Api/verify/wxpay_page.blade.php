...
<div class="bodyIn">
    <div class="wxTop" style="background: {{$appInfoData['bg_color']}}">
        <div class="wxTopMiddle"><h1>微信认证</h1></div>
    </div>
    <div class="wxZsxmBox">
        ...
        <div class="wxZsxmIn">
            <h2>微信授权:</h2>
            <input type="text" value="微信授权" id="wx_name" class="text2" onBlur="if(this.value=='') this.value='微信授权';"
                   onFocus="if(this.value=='微信授权') this.value='';"/>
            <span><a href="javascript:void(0);" onclick="auth()">去授权<i class="you">&#xe697;</i></a></span>
        </div>
        <div class="wxZsxmSub"><input type="submit" style="background: {{$appInfoData['bg_color']}}" value="确定认证" class="submit1"/></div>
    </div>
...
<script>
    ...
    function calllback(obj) {
        tingwan.showToast(obj);
        unionid = JSON.parse(obj).unionid;
        $('#wx_name').val(JSON.parse(obj).screen_name);
        tingwan.showToast('成功,请点击确定认证!');
    }
    ...
    $('.submit1').click(function () {
        ...
        if (unionid == null) {
            tingwan.showToast('微信授权未成功!');
        }
        $.ajax({
            type: 'post',
            url: 'http://newadmin.tingwan.com/api/user/verify',
            data: {realName: realName, idCard: idCard, type: 'wxpay', unionid: unionid},
            dataType: "json",
            headers: {'token': token},
            success: function (data) {
                ...
                if (data.code == 200) {
                    window.location.href = '/api/pfc/page';
                }
            },
        });
        return false;
