...
<div class="page-container">
    <form action="" method="post" class="form form-horizontal" id="form-article-add">
        <!-- 失败提示框 -->
        @if(Session::has('news_add_fail'))
            <div class="Huialert Huialert-danger"><i class="Hui-iconfont">&#xe6a6;</i>{{Session::get('news_add_fail')}}</div>
        @endif
        <div class="row cl">
            ...
            <span class="select-box">
              <select name="app_id" class="select" id="app_select">
                @foreach($app_infos as $app_info)
                      <option value="{{$app_info->app_id}}">{{$app_info->app_name}}</option>
                  @endforeach
              </select>
            </span>
            </div>
        ...
        <div class="row cl">
            ...
            <div class="formControls col-xs-8 col-sm-9">
                <textarea name="description" cols="200" rows="20" style="color:#999999"></textarea>
            </div>
        </div>
        <div class="row cl">
