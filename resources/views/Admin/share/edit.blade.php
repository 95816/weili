...
<div class="page-container">
    <form action="" method="post" class="form form-horizontal" id="form-article-add" enctype="multipart/form-data">
        <!-- 失败提示框 -->
        @if(Session::has('share_edit_fail'))
            <div class="Huialert Huialert-danger"><i class="Hui-iconfont">&#xe6a6;</i>{{Session::get('share_edit_fail')}}</div>
        @endif
        ...
        <div class="row cl">
            ...
            <div class="formControls col-xs-8 col-sm-9">
                <input type="text" class="input-text" value="{{$data->title}}" placeholder="分享标题必填" id="" name="title">
            </div>
        </div>
        <div class="row cl">
            <label class="form-label col-xs-4 col-sm-2"><span class="c-red">*</span>分享URL：</label>
            <div class="formControls col-xs-8 col-sm-9">
                <input type="text" class="input-text" value="{{$data->url}}" placeholder="分享URL必填" id="" name="url">
            </div>
        </div>
        <div class="row cl">
            <label class="form-label col-xs-4 col-sm-2">分享内容：</label>
            <div class="formControls col-xs-8 col-sm-9">
                <textarea name="content" cols="200" rows="20" style="color:#999999">{{$data->content}}</textarea>
            </div>
        </div>
        <div class="row cl">
            <label class="form-label col-xs-4 col-sm-2">分享图片：</label>
            <div class="formControls col-xs-8 col-sm-9">
                <img class="layui-upload-img img-upload-view" id="previewImg" width="120px" height="120px" src="{{$data->image}}">
        ...
        <div class="row cl">
            ...
            <div class="formControls col-xs-8 col-sm-9 skin-minimal">
                <div class="radio-box">
                    <input name="status" type="radio" value="1" id="sex-1" {{$data->status==1?'checked':''}}>
                    <label for="sex-1">启用</label>
                </div>
                <div class="radio-box">
                    <input type="radio" id="sex-2" value="2" name="status" {{$data->status==2?'checked':''}}>
