            ...
            <form action="" method="post" class="form form-horizontal" id="form-admin-role-add">
                ...
                <div class="row cl">
                    ...
                    <div class="formControls col-xs-8 col-sm-9 skin-minimal">
                        <div class="radio-box">
                            <input name="status" type="radio" id="sex-1" {{$role->status == 1 ? 'checked=checked': ''}} value="1">
                            <label for="sex-1">启用</label>
                        </div>
                        <div class="radio-box">
                            <input type="radio" id="sex-2" name="status" {{$role->status == 2 ? 'checked=checked': ''}} value="2">
                                    ...
                                    <dl class="cl permission-list2">
                                        ...
                                        <dd>
                                            @foreach($val['childs'] as $k=>$v)
                                                <label class="">
                                                    <input type="checkbox" value="{{$v['id']}}" @if(in_array($v['id'],$power_new_arr)) checked
                                                           @endif name="power[]" id="user-Character-0-0-{{$k}}">
                                                    {{$v['name']}}</label>
                                            @endforeach
                                        </dd>
                                    </dl>
